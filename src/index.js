...
module.exports = async () => {
  console.log(
    FgCyan,
    `INIT_STANDARDS VERSION: ${initStandardsPackage.version}`
  );
  ...
  const requiredDeps = resolveDeps(
    targetPackageJSON,
    initStandardsPackage.devDependencies
  );
  ...
  if (!requiredDeps.length) {
    ...
    const depsWithVersions = requiredDeps
      .map(dep => {
        return `${dep}@${initStandardsPackage.devDependencies[dep].substring(
          1
        )}`;
      })
      .join(' ');
  ...
  try {
    ...
    modifiedPackage.husky = initStandardsPackage.husky;
    if (modifiedPackage.scripts) {
      modifiedPackage.scripts.lint = initStandardsPackage.scripts.lint;
    } else {
  ...
  console.log(
    FgGreen,
    'Configurations for linting/prettier standards successfully installed.'
  );
  process.exit();
};
